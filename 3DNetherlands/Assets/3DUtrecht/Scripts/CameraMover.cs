...
public class CameraMover : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            var newpos = transform.position + ray.direction.normalized * (Input.mouseScrollDelta.y * 40f);
    ...
    {
        //float horpos = transform.eulerAngles.y + (Input.GetAxis("RightStickX") * Time.deltaTime * RotationSpeed);
        //float vertpos = transform.eulerAngles.x - (Input.GetAxis("RightStickY") * Time.deltaTime * RotationSpeed);
        //if (vertpos < 271f && vertpos > 265f) vertpos = -89f;
        //else if (vertpos > 89f && vertpos < 95f) vertpos = 89f;
        //transform.eulerAngles = new Vector3(vertpos, horpos, 0);
